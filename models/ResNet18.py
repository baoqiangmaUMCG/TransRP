...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.fc_img = nn.Linear(block_inplanes[3] * block.expansion, 64) if feed_forward else None
        self.relu_img = nn.ReLU(inplace=True)
        self.fc_clc = nn.Linear(7, 64) 
        self.relu_clc = nn.ReLU(inplace=True)
        self.fc = nn.Linear(128,  num_classes) if feed_forward else None
    ...
    def forward(self, x: torch.Tensor, x_clc: torch.Tensor) -> torch.Tensor:
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        ...
        x = self.relu_img(x)
        # clc 
        x_clc = self.fc_clc(x_clc)
        x_clc = self.relu_clc(x_clc)

        if self.fc is not None:
            #x = self.fc(x)
            x = self.fc(torch.cat((x, x_clc), 1)) # new
...
def resnet10_clc(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
...
def ResNet18_m3(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
...
def resnet34_clc(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
...
def resnet50_clc(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
...
def resnet101_clc(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
...
def resnet152_clc(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
...
def resnet200_clc(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
